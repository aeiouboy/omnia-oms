#!/usr/bin/env python3
"""
Delete Created User Stories - Remove all user stories created by our scripts
...
def main():
    """Delete only the user stories created by our scripts"""
    ...
    # Confirm deletion
    confirm = input(f"\n⚠️  Are you sure you want to delete {len(story_ids)} user stories? (yes/no): ")
    ...
    if deleted_count > 0:
        ...
        # Ask if user wants to clear the work item mapping
        clear_mapping = input("\n🧹 Clear the work item mapping file? (yes/no): ")
        if clear_mapping.lower() == 'yes':
            # Clear only the stories section, keep epics
            mapping['stories'] = {}

#!/usr/bin/env python3
"""
Fix the proper formatting for all Azure DevOps work items
This script will parse the user stories correctly and format them properly
"""
...
class ProperStoryParser:
    """Proper parser that maintains the correct format"""
    
    @staticmethod
    def parse_markdown_file(file_path: str) -> dict:
        """Parse the markdown file correctly"""
        ...
        for i in range(1, len(story_sections), 3):
            if i+2 < len(story_sections):
                ...
                parsed_story = ProperStoryParser._parse_story_content(story_id, story_title, story_content)
                if parsed_story:
    ...
    def _parse_story_content(story_id: str, title: str, content: str) -> dict:
        """Parse individual story content properly"""
        story = {
            'id': story_id,
            'title': title,
            'priority': 'P1',
...
def main():
    ...
    # Fix work items with proper formatting
    print(f"🛠️  Fixing work item formatting...")
    ...
    for story_id, work_item_id in story_mapping.items():
        if story_id in stories:
            ...
            try:
                # Create proper HTML description
